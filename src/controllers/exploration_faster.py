"""Fast hybrid controller: deterministic racing line plus a trainable RL correction residual.

Iterative hybrid RL + racing-line optimization plan: the geometric planner and
per-sector racing line/speed schedule below are the deterministic backbone
(fast, safe, seed-general). `_ResidualGeometricPlanner` applies a small
learned residual on top of that backbone at every decision tick, and
`_hazard_score` widens that residual's authority when sensors indicate an
unusual situation (wall/car contact, large tracking error, unexpected yaw
rate, lost track visibility) so the learned component leans in exactly when
the deterministic line is least trustworthy, and otherwise stays out of the
way. All of this is controlled by `_TuningParams`: `create_controller(tuning=...)`
accepts an override for fast in-process search (see
scripts/train_hybrid_residual.py and scripts/evaluate_controller.py), and
`hybrid_tuning.json`, if present next to this file (or at the path named by
the `HYBRID_TUNING_PATH` environment variable), supplies a persisted default
so this remains a standalone, deterministic-at-inference-time module when no
override is given -- every `_TuningParams` field defaults to exactly the
original hand-tuned constants, so behavior is unchanged until something is
actually trained.
"""
...
class _TuningParams:
    """Every knob a training script may search over.

    Field defaults exactly reproduce the original hand-tuned constants above,
    so `_TuningParams()` (used whenever no override or tuning file is
    present) drives the controller identically to before this became
    trainable. `actor_weights`/`action_residual_scales`/
    `hazard_residual_scale_max` are the Phase 3/4 RL correction policy;
    the remaining fields are the Phase 2/5 racing line and speed schedule,
    split into `launch_*` (first lap) and `recurring_*` (every lap after)
    since the original constants already differed slightly between the two.
    """

    actor_weights: tuple[tuple[float, ...], tuple[float, ...]] = _ACTOR_WEIGHTS
    action_residual_scales: tuple[float, float] = _ACTION_RESIDUAL_SCALES
    hazard_residual_scale_max: float = 1.0
    launch_corner_offsets_m: tuple[float, ...] = _LEARNED_CORNER_OFFSETS_M
    launch_curve_speed_gains: tuple[float, ...] = _ROBUST_LIMIT_CURVE_SPEED_GAINS
    launch_straight_speed_mps: float = _AGGRESSIVE_STRAIGHT_SPEED_MPS
    recurring_corner_offsets_m: tuple[float, ...] = _RECOMBINED_INSIDE_OFFSETS_M
    recurring_curve_speed_gains: tuple[float, ...] = (*_SECTOR_2_LIMIT_GAINS[:4], 0.78)
    recurring_straight_speed_mps: float = 39.0


_TUNING_PATH = Path(__file__).with_name("hybrid_tuning.json")


def _coerce_tuning_overrides(raw: dict[str, object]) -> dict[str, Any]:
    overrides: dict[str, Any] = {}
    if "actor_weights" in raw:
        rows = cast(list[list[float]], raw["actor_weights"])
        overrides["actor_weights"] = tuple(tuple(float(value) for value in row) for row in rows)
    for key in (
        "action_residual_scales",
        "launch_corner_offsets_m",
        "launch_curve_speed_gains",
        "recurring_corner_offsets_m",
        "recurring_curve_speed_gains",
    ):
        if key in raw:
            overrides[key] = tuple(float(value) for value in cast(list[float], raw[key]))
    for key in ("hazard_residual_scale_max", "launch_straight_speed_mps", "recurring_straight_speed_mps"):
        if key in raw:
            overrides[key] = float(cast(float, raw[key]))
    return overrides


def _load_default_tuning() -> _TuningParams:
    override_path = os.environ.get("HYBRID_TUNING_PATH")
    path = Path(override_path) if override_path else _TUNING_PATH
    if not path.exists():
...
_DEFAULT_TUNING = _load_default_tuning()
...
_HAZARD_RECOVERY_SECONDS = 0.5


def _hazard_score(sensors: RobotSensors, seconds_since_contact: float) -> float:
    """0..1 "how unusual is this tick" signal, built only from public sensors.

    Phase 4 of the improvement plan: the deterministic line is trustworthy on
    a normal tick, but should hand more authority to the learned residual
    when something is off -- active or just-ended wall/car contact, being
    far from where the camera expects the track to be, spinning faster than
    the current curve would predict (loss of traction / unexpected steering
    deviation), or losing the track entirely.
    """
    contact_hazard = (
        1.0
        if sensors.contact.wall > 0.0 or sensors.contact.robot > 0.0
        else _clamp(1.0 - seconds_since_contact / _HAZARD_RECOVERY_SECONDS, 0.0, 1.0)
    )
    tracking_error = max(
        abs(sensors.camera.center_offset_m) / 2.4,
        abs(sensors.camera.heading_error_degrees) / 55.0,
    )
    tracking_hazard = _clamp((tracking_error - 0.55) / 0.75, 0.0, 1.0)
    yaw_hazard = _clamp((abs(sensors.imu.yaw_rate_degrees_per_s) - 90.0) / 90.0, 0.0, 1.0)
    visibility_hazard = 0.0 if sensors.camera.visible else 1.0
    return max(contact_hazard, tracking_hazard, yaw_hazard, visibility_hazard)
...
class _ResidualGeometricPlanner:
    ...
    def __init__(self, *, tuning: _TuningParams = _DEFAULT_TUNING) -> None:
        ...
        self._seconds_since_contact = _HAZARD_RECOVERY_SECONDS
    ...
    def __call__(self, sensors: RobotSensors) -> RobotCommand:
        ...
        if sensors.contact.wall > 0.0 or sensors.contact.robot > 0.0:
            self._seconds_since_contact = 0.0
        else:
            self._seconds_since_contact += sensors.dt_s
        ...
        hazard = _hazard_score(sensors, self._seconds_since_contact)
        hazard_scale = 1.0 + (self._tuning.hazard_residual_scale_max - 1.0) * hazard
        throttle, steer = _policy_action_means(features, base_command, self._tuning, hazard_scale)
...
def _policy_action_means(
    ...
    tuning: _TuningParams,
    hazard_scale: float,
) -> tuple[float, float]:
    throttle_scale = tuning.action_residual_scales[0] * hazard_scale
    steer_scale = tuning.action_residual_scales[1] * hazard_scale
    throttle_residual = throttle_scale * tanh(_dot(tuning.actor_weights[0], features))
    steer_residual = steer_scale * tanh(_dot(tuning.actor_weights[1], features))
...
class _SectorController:
    ...
    def __init__(
        ...
        *,
        ...
        tuning: _TuningParams = _DEFAULT_TUNING,
    ) -> None:
        ...
        self._stable = _ResidualGeometricPlanner(tuning=tuning)
        self._base = _ResidualGeometricPlanner(tuning=tuning)
...
def _new_sector_controller(policy: _LaunchPolicy, tuning: _TuningParams) -> _SectorController:
    return _SectorController(
        corner_offsets_m=tuning.launch_corner_offsets_m,
...
def _select_launch_policy(sensors: RobotSensors, tuning: _TuningParams) -> _LaunchPolicy:
    heading = sensors.imu.heading_degrees
    front = sensors.wall_lidar.front_m
    far_offset = sensors.camera.lookahead_offsets_m[-1]
    ...
    return _LaunchPolicy(
        tuning.launch_curve_speed_gains,
        tuning.launch_straight_speed_mps,
        _AGGRESSIVE_BRAKING_BOUNDARIES_M,
        None,
    )
    ...
    def __call__(self, sensors: RobotSensors, progress: float) -> RobotCommand:
        if self._selected is None:
            ...
            if progress < 1.0:
                ...
                path = _EARLY_PATH
            policy = _select_launch_policy(sensors, self._tuning)
            self._selected = _GuardedLaunchController(path, _new_sector_controller(policy, self._tuning))
...
class _RecurringLapController:
    ...
    def __init__(self, tuning: _TuningParams = _DEFAULT_TUNING) -> None:
        ...
        self._sector = _SectorController(
            corner_offsets_m=tuning.recurring_corner_offsets_m,
            curve_speed_gains=tuning.recurring_curve_speed_gains,
            speed_boundaries_m=_AGGRESSIVE_BRAKING_BOUNDARIES_M,
            straight_speed_numerator=tuning.recurring_straight_speed_mps,
            tuning=tuning,
        )
...
class Controller:
    ...
    def __init__(self, tuning: _TuningParams | None = None) -> None:
        self._tuning = tuning if tuning is not None else _DEFAULT_TUNING
        self._state = _RaceState.LAUNCH
        self._launch = _FirstCornerLaunchController(self._tuning)
    ...
    def __call__(self, sensors: RobotSensors) -> RobotCommand:
        ...
        if self._started_beyond_merge:
            ...
            self._recurring = _RecurringLapController(self._tuning)
            self._state = _RaceState.RECURRING
            return self._recurring(sensors)
        return self._launch(sensors, progress)

    def copy_for_car(self) -> Controller:
        return Controller(self._tuning)


def create_controller(tuning: _TuningParams | None = None) -> Controller:
    return Controller(tuning)
