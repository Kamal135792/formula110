"""Search earlier throttle application on V73's wide final-corner exit."""

from __future__ import annotations

from statistics import mean

from benchmark_controller import summarize
from train_v0 import SoloEpisodeResult, SoloRaceRunner

from controllers.v72 import Controller

SEEDS = (3, 10, 31, 64, 96, 100)


def main() -> int:
    runner = SoloRaceRunner()
    rows: list[tuple[int, float, float, float, float]] = []
    candidates: list[tuple[float | None, float]] = [
        (None, 165.0),
        (0.90, 154.0),
        (0.95, 156.0),
        (1.00, 157.0),
        (1.00, 158.0),
        (1.00, 159.0),
        (1.00, 160.0),
    ]
    try:
        for throttle_floor, start_m in candidates:
            results: list[SoloEpisodeResult] = [
                runner.run(
                    Controller(
                        straight_speed_mps=39.0,
                        exit_throttle_floor=throttle_floor,
                        exit_throttle_start_m=start_m,
                    ),
                    seed=seed,
                    duration_seconds=20.0,
                )
                for seed in SEEDS
            ]
            summary = summarize(results)
            laps = [result.best_lap_seconds for result in results if result.best_lap_seconds is not None]
            mean_lap = mean(laps) if laps else 999.0
            damage_count = sum(result.damage > 0.0 for result in results)
            rank_floor = -1.0 if throttle_floor is None else throttle_floor
            rows.append((damage_count, mean_lap, -summary.mean_progress_m, rank_floor, start_m))
            floor_text = "base" if throttle_floor is None else f"{throttle_floor:.2f}"
            print(
                f"floor={floor_text:>4} start={start_m:.0f} damage_seeds={damage_count} "
                f"max_damage={max(result.damage for result in results):.6f} "
                f"progress={summary.mean_progress_m:.2f} min={summary.minimum_progress_m:.2f} "
                f"mean_lap={mean_lap:.4f} best={summary.best_lap_seconds}"
            )
    finally:
        runner.close()
    print("ranked (hard safety, then lap time):")
    for damage_count, mean_lap, negative_progress, rank_floor, start_m in sorted(rows):
        floor_text = "base" if rank_floor < 0.0 else f"{rank_floor:.2f}"
        print(
            f"floor={floor_text:>4} start={start_m:.0f} damage_seeds={damage_count} "
            f"mean_lap={mean_lap:.4f} progress={-negative_progress:.2f}"
        )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
