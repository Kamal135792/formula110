"""Search outside-inside-outside final-corner paths behind V71's safe baseline."""
...
APEX_STEERS = (0.02, 0.04, 0.06)
EXIT_STEERS = (-0.05, -0.07, -0.08, -0.09, -0.11)
...
def main() -> int:
    ...
    try:
        for apex_steer in APEX_STEERS:
            for exit_steer in EXIT_STEERS:
                results: list[SoloEpisodeResult] = [
                    runner.run(
                        Controller(apex_steer=apex_steer, exit_steer=exit_steer),
                        seed=seed,
                ...
                rows.append((damage_count, mean_lap, -summary.mean_progress_m, apex_steer, exit_steer))
                print(
                    f"apex={apex_steer:+.2f} exit={exit_steer:+.2f} damage_seeds={damage_count} "
    ...
    for damage_count, mean_lap, negative_progress, apex_steer, exit_steer in sorted(rows):
        print(
            f"apex={apex_steer:+.2f} exit={exit_steer:+.2f} damage_seeds={damage_count} "
